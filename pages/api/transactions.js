  ...
  if (
    ...
    if (
      ...
      if (raw.data !== null && raw.data.getUser.userName === userName) {
        do {
          raw = await API.graphql(
            graphqlOperation(queries.listPayments, {
              filter: {
                and: [
                  { date: { between: [startDate, stopDate] } },
                  { paymentAgentId: { eq: agentId } }
                ]
              },
              limit: 10000,
              nextToken: nextToken
          ...
          nextToken = raw.data.listPayments.nextToken;
          raw.data.listPayments.items.map(value => results.push(value));
        } while (nextToken);
